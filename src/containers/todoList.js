...
class TodoList extends Component {
    ...
    handleDeleteTodo(id) {
        firebaseApp.database().ref(`/Users/${this.props.currentUser.userUid}/todos/${id}`).remove();
    }
    ...
    handleSaveTodo(id) {
        firebaseApp.database().ref(`/Users/${this.props.currentUser.userUid}/todos/${id}`).once('value', data => {
            let thisTodo = data.val();
            thisTodo.todoWork = this.state.editTodoWork;
            thisTodo.editStatus = false;
            firebaseApp.database().ref(`/Users/${this.props.currentUser.userUid}/todos/${id}`).set(thisTodo);
        })
    }
    ...
    render() {
        ...
        if (this.props.userAuthenticationStatus) {
            <Redirect to={from} />
        }
        ...
        return (
            ...
            <div id="WorkList_div">
                ...
                {
                    this.props.currentUserTodos.length < 1 ? <h3 id="message2">Hurray! No Work Pending</h3> :
                        <div>
                            <h2 id="message">Here is the Work List</h2>
                            <ul id="workList">
                                {
                                    this.props.currentUserTodos.map((todo, ind) =>
...
const mapStateToProps = state => {
    return {
        ...
        currentUserTodos: state.userTodos.todos
    }
}
